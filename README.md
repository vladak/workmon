...
My goal is to provide the following functionality:
  - determine the hours I work (per day/week)
    ...
    - send an alert if I work too much in a given week (via Grafana + PagerDuty)
...
- Raspberry Pi Zero 2 W (side note: out of stock on Adafruit, can be order 1 piece per customer locally)
- [RPIZ CT3V1 HAT](http://lechacal.com/wiki/index.php?title=RPIZ_CT3V1) with Current sensors
  - to detect my presence in fron of the computer. This works with sufficient degree of precision because my display has the saver set just to 3 minutes.
    - while the presence could be detected in software, I wanted to avoid any changes whatsover to the software of my company provided laptop
